...
func (s *Scheduler) scheduleOne() {
	...
	go func() {
		...
		var needUpdate bool
		for i, volume := range assumed.Spec.Volumes {
			localDisk := volume.LocalDisk
			if localDisk != nil {
				needUpdate = true
				glog.V(3).Infof("===LocalDisk : %s", localDisk.LocalPath)
				pod.Spec.Volumes[i].LocalDisk.LocalPath = localDisk.LocalPath
			}
		}
		if needUpdate {
			glog.V(3).Infof("Update pod %s's spec : %+v", pod.Name, *pod)
			...
			if p, err := s.config.KubeClient.Pods(pod.Namespace).Update(pod); err != nil {
